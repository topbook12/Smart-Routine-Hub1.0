...
function MyNoticeRow({ notice, index }: { notice: Notice; index: number }) {
  ...
  const [editForm, setEditForm] = useState({
    ...
    affectedProgram: notice.affectedProgram ?? "",
    affectedSemester: notice.affectedSemester ? String(notice.affectedSemester) : "",
  });
    ...
    mutationFn: async () => {
      const res = await fetch("/api/notices", {
        ...
        body: JSON.stringify({
          ...
          affectedProgram: editForm.affectedProgram || null,
          affectedSemester: editForm.affectedSemester ? Number(editForm.affectedSemester) : null,
        }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => null);
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="grid gap-3">
            <div>
              ...
              <Input value={editForm.title} onChange={(e) => setEditForm({ ...editForm, title: e.target.value })} className="mt-1.5 h-10" />
            </div>
            <div>
              <Label className="text-xs">Content</Label>
              <Textarea value={editForm.content} onChange={(e) => setEditForm({ ...editForm, content: e.target.value })} className="mt-1.5" rows={4} />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <Label className="text-xs">Category</Label>
                <Select value={editForm.category} onValueChange={(v) => setEditForm({ ...editForm, category: v as NoticeCategory })}>
              ...
              <div>
                ...
                <Select value={editForm.affectedProgram || "none"} onValueChange={(v) => setEditForm({ ...editForm, affectedProgram: v === "none" ? "" : v })}>
                  <SelectTrigger className="mt-1.5 h-10"><SelectValue /></SelectTrigger>
                  <SelectContent>
            ...
            <div className="flex items-center justify-between gap-2">
              <div className="flex-1">
                <Label className="text-xs">Semester (optional)</Label>
                <Input type="number" min={1} max={8} value={editForm.affectedSemester} onChange={(e) => setEditForm({ ...editForm, affectedSemester: e.target.value })} className="mt-1.5 h-10" placeholder="e.g. 5" />
              </div>
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg border h-10 mt-auto">
                <Pin className="h-3.5 w-3.5 text-amber-500" />
                <Label className="text-xs">Pin</Label>
                <Switch checked={editForm.isPinned} onCheckedChange={(c) => setEditForm({ ...editForm, isPinned: c })} />
              </div>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditOpen(false)}>Cancel</Button>
            <Button onClick={() => editMutation.mutate()} disabled={editMutation.isPending} className="btn-3d btn-ink gap-1.5">
